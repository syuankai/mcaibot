...
io.use((socket, next) => {
    ...
    if (token && auth.verifyToken(token)) return next();
    next(new Error('未授權'));
});
...
if (!auth.isPasswordSet()) {
    console.log('\x1b[33m[認證] 尚未設定密碼，首次訪問網頁時可設定\x1b[0m');
}

app.get('/api/auth/status', (req, res) => {
    res.json({ setup: !auth.isPasswordSet() });
});

app.post('/api/auth/setup', (req, res) => {
    if (auth.isPasswordSet()) {
        return res.status(403).json({ error: '密碼已設定' });
    }
    const { password } = req.body || {};
    if (!password || password.length < 4) {
        return res.status(400).json({ error: '密碼長度至少 4 位' });
    }
